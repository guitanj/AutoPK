...
def startPK(num,weightb,weightw,spendTime):
    ...
    pbscmd = 'C:\\Go\\1130fastexit-tensor-accum\\leelaz.exe -g --noponder --precision half -wC:\\Go\\weight\\' \
             +weightb+' --gpu 1 -p 100000'
    ...
    pwscmd = 'C:\\Go\\1130fastexit-tensor-accum\\leelaz.exe -g --noponder -t 1 --precision half --batchsize 4 -wC:\\Go\\weight\\' \
             +weightw+' --gpu 1 -p 100000'
...
if __name__ == "__main__":
    ...
    spendTime = 1 #可修改：设定pk时间
    weightb='elfv1' #可修改：执黑权重
    weightw='elfv1' #可修改：执白权重
    while spendTime <= 1: #可修改：设定pk上限时间
